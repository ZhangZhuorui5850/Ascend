...
export function KineticCalendar({ calendars, events: initialEvents, exams, lists, tasks: initialTasks, timeZone, today }: {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.controlRail}>
      ...
      <div className={styles.modeControl}>{(["month","agenda","focus"] as CalendarMode[]).map((item) => <button aria-pressed={mode === item} className={mode === item ? styles.modeActive : ""} key={item} onClick={() => setMode(item)} type="button">{mode === item ? <m.i layoutId="kinetic-calendar-mode" transition={motionContract.row.reorder} /> : null}<span>{item === "month" ? "月场" : item === "agenda" ? "议程" : "负载"}</span></button>)}</div>
    ...
    <AnimatePresence mode="wait" initial={false}>
      <m.main animate={{opacity:1,y:0,filter:"blur(0px)"}} className={styles.stage} exit={reduceMotion ? undefined : {opacity:0,y:-12,filter:"blur(8px)"}} initial={reduceMotion ? false : {opacity:0,y:18,filter:"blur(10px)"}} key={mode}>
    ...
    <AnimatePresence>{toast ? <m.div animate={{opacity:1,y:0,scale:1}} className={styles.toast} exit={{opacity:0,y:12,scale:.95}} initial={{opacity:0,y:24,scale:.94}} role="status"><Sparkles size={14}/>{toast}</m.div> : null}</AnimatePresence>
  </div>;
}
