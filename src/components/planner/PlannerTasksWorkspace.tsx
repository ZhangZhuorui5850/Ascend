...
import { plannerOptimisticReducer, reconcilePlannerSelection } from "@/components/planner/planner-optimistic";
...
import {
  filterPlannerTaskView,
  isPlannerTaskView,
  type PlannerTaskView,
  type PlannerTaskViewContext,
} from "@/lib/planner/task-views";
...
export function PlannerTasksWorkspace({
  ...
  const [captureListId, setCaptureListId] = useState(lists[0]?.id ?? "");
  const [dueDate, setDueDate] = useState("");
  const [selectedId, setSelectedId] = useState<string | null>(
    () => filterPlannerTaskView(initialTasks, initialView, taskViewContext, { listId: initialListId })[0]?.id ?? null,
  );
  ...
  const visibleTasks = useMemo(
    () => filterPlannerTaskView(tasks, activeView, taskViewContext, { listId: activeListId }),
    [activeListId, activeView, taskViewContext, tasks],
  );
  const visibleSelectedId = visibleTasks.some((task) => task.id === selectedId) ? selectedId : null;
  ...
  useEffect(() => {
    const restoreViewFromLocation = () => {
      ...
      const requestedListId = params.get("list") ?? undefined;
      const nextView = isPlannerTaskView(requestedView) ? requestedView : "inbox";
      const nextListId =
        requestedListId && lists.some((list) => list.id === requestedListId) ? requestedListId : undefined;
      setActiveView(nextListId ? "all" : nextView);
      setActiveListId(nextListId);
    };
    window.addEventListener("popstate", restoreViewFromLocation);
    return () => window.removeEventListener("popstate", restoreViewFromLocation);
  }, [lists]);

  function changeTaskView(nextView: PlannerTaskView, nextListId?: string): void {
    if (nextView === activeView && nextListId === activeListId) return;
  ...
  function navigateTasks(taskId: string, direction: -1 | 1): void {
    ...
    const next = visibleTasks[Math.min(Math.max(index + direction, 0), visibleTasks.length - 1)];
    if (!next) return;
  ...
  function addTask(event: FormEvent<HTMLFormElement>, parentTaskId?: string): void {
    ...
    const nextTitle = parentTaskId
      ...
      : title.trim();
    if (!nextTitle || !captureListId) return;
    const submittedListId = captureListId;
  ...
  function removeTask(task: PlannerTask): void {
    const sourceIndex = tasks.findIndex((item) => item.id === task.id);
    ...
    startTransition(async () => {
      ...
      if (result.ok && result.entity) {
        ...
        notify(`已将“${task.title}”移入回收站`, "success", {
          actionLabel: "撤销",
          undo: () => undoDelete(task, deleted, sourceIndex),
        });
      } else {
        const rollback = { type: "restore", task, index: sourceIndex } as const;
        applyOptimistic(rollback);
  ...
  function runBatch(patch: { status?: "completed"; listId?: string; deleted?: boolean }): void {
    const selectedTasks = visibleTasks.filter((task) => checked.has(task.id));
    ...
    startTransition(async () => {
      ...
      if (result.ok && result.entities) {
        ...
        for (const { index, task } of rollback) {
          applyOptimistic(
            patch.deleted ? { type: "restore", task, index } : { type: "patch", id: task.id, patch: task },
          );
        }
  ...
  function createList(event: FormEvent<HTMLFormElement>): void {
    ...
    if (!name) return;
    startTransition(async () => {
      const result = await runPlannerMutation(() => createTaskListAction({ name }), "网络异常，清单创建失败");
      if (result.ok && result.entity) {
        setCaptureListId(result.entity.id);
        setNewListName("");
  ...
  function createLabel(event: FormEvent<HTMLFormElement>): void {
    ...
    if (!name) return;
    startTransition(async () => {
      const result = await runPlannerMutation(() => createPlannerLabelAction({ name }), "网络异常，标签创建失败");
      if (result.ok && result.entity) {
  ...
  return (
    <MotionProvider>
      <PlannerMobileNavigation onViewChange={changeTaskView} view={activeView} />
      <div
        className={styles.workspace}
        data-planner-view={activeView}
        data-planner-workspace="tasks"
      >
        <PlannerSidebar
        ...
        <main className={styles.main}>
          ...
          <PlannerQuickCapture
            ...
            dueDate={dueDate}
            listId={captureListId}
            lists={lists}
            onDueDateChange={setDueDate}
            onListChange={setCaptureListId}
            onSubmit={addTask}
          ...
          <LayoutGroup id="planner-task-selection">
            <PlannerTaskGroups
              checked={checked}
              groups={grouped}
              key={activeListId ? `list:${activeListId}` : `view:${activeView}`}
              onCheck={checkTask}
              onOpen={openTask}
...
function groupTasks(tasks: PlannerTask[]): Array<[string, PlannerTask[]]> {
  ...
  for (const task of tasks) {
    const key = task.scheduled_start_at ? "已排期" : task.due_date || task.due_at ? "有到期" : "待安排";
...
function scheduledParts(task: PlannerTask, timeZone: string): { date: string; time: string } {
  ...
  const parts = Object.fromEntries(
    formatter
      .formatToParts(new Date(task.scheduled_start_at))
