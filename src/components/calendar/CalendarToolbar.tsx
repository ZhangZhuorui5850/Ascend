  ...
  return (
    <header className={styles.toolbar}>
      ...
      <div className={styles.toolbarGroup} data-calendar-navigation>
        <button aria-label="上一时间段" onClick={() => onNavigate("prev")} type="button">
          <ChevronLeft size={16} />
        </button>
        <button aria-label="回到今天" onClick={() => onNavigate("today")} type="button">
          今天
        </button>
        <button aria-label="下一时间段" onClick={() => onNavigate("next")} type="button">
      ...
      <div className={styles.toolbarActions}>
        ...
        <button
          aria-label="打开待排任务"
          aria-pressed={activeContext === "inbox"}
          data-active={activeContext === "inbox"}
          onClick={(event) => onContext("inbox", event.currentTarget)}
          type="button"
        >
          <Clock3 size={15} />
          <span>待排</span>
        ...
        <button
          aria-label="新建事件"
          aria-pressed={activeContext === "composer"}
          className="primaryButton"
          data-active={activeContext === "composer"}
          onClick={(event) => onContext("composer", event.currentTarget)}
          type="button"
        >
          <Plus size={15} />
          <span>新建</span>
