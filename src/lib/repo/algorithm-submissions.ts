...
import { JUDGE_LANGUAGES, type JudgeGatewayResult, type JudgeLanguage, type JudgeStatus } from "../judge-gateway";
import {
...
const TERMINAL_STATUSES = new Set<JudgeStatus>(["AC", "WA", "TLE", "MLE", "RE", "CE", "JE", "CANCELLED"]);
...
export type AlgorithmCodeVersion = {
  id: string;
  problemId: number;
  language: JudgeLanguage;
  revision: number;
...
export type SaveAlgorithmDraftResult = {
  codeBlobId: string;
  sha256: string;
  revision: number;
  savedAt: string;
...
export type AlgorithmDraftConflict = {
  revision: number;
  sha256: string;
  updatedAt: string;
  deviceId: string;
...
export class AlgorithmDraftConflictError extends Error {
  readonly code = "DRAFT_CONFLICT";
  readonly status = 409;
  readonly current: AlgorithmDraftConflict;

  constructor(current: AlgorithmDraftConflict) {
    super("云端草稿已经更新");
    this.name = "AlgorithmDraftConflictError";
...
export function saveAlgorithmDraft(
  ...
  input: {
    ...
    baseRevision?: number;
    operationId?: string;
    versionKind?: "autosave" | "manual" | "sample_passed" | "submitted" | "accepted" | "retest";
    label?: string;
  },
  key: JudgeCodeKey,
): SaveAlgorithmDraftResult {
  requirePluginEnabled(db, scope, "algorithms");
  requireEditableProblem(db, scope, input.problemId, input.language);
  const sha256 = createHash("sha256").update(input.sourceCode, "utf8").digest("hex");
  return db.transaction(() => {
    const operationId = normalizeDraftOperationId(input.operationId);
    if (operationId) {
      const replay = db.prepare(`
        SELECT problem_id AS problemId, language, base_revision AS baseRevision,
               saved_revision AS savedRevision, code_blob_id AS codeBlobId,
               code_sha256 AS sha256, created_at AS savedAt
        FROM algorithm_draft_operations
        WHERE workspace_id = ? AND operation_id = ?
      `).get(scope.workspaceId, operationId) as {
        problemId: number;
        language: string;
        baseRevision: number;
      ...
      if (replay) {
        if (
          ...
          || (input.baseRevision !== undefined && replay.baseRevision !== input.baseRevision)
        ) {
          throw new Error("同一草稿幂等键不能用于不同请求");
        }
        return {
          codeBlobId: replay.codeBlobId,
          sha256: replay.sha256,
          revision: replay.savedRevision,
          savedAt: replay.savedAt,
        };
      }
    }

    const existing = readDraftHead(db, scope, input.problemId, input.language);
    const baseRevision = normalizeDraftBaseRevision(input.baseRevision, existing?.revision ?? 0);
    if (existing?.sha256 === sha256) {
      if (operationId) recordDraftOperation(db, scope, {
        problemId: input.problemId,
        language: input.language,
        operationId,
        baseRevision,
        savedRevision: existing.revision,
        codeBlobId: existing.codeBlobId,
        sha256,
      });
      return {
        codeBlobId: existing.codeBlobId,
    ...
    if (baseRevision !== (existing?.revision ?? 0)) {
      throw new AlgorithmDraftConflictError({
    ...
    db.prepare(
      ...
      INSERT INTO algorithm_code_drafts
        (workspace_id, problem_id, language, code_blob_id, revision, last_device_id)
      VALUES (?, ?, ?, ?, ?, ?)
    ...
    `,
    ).run(scope.workspaceId, input.problemId, input.language, saved.id, revision, input.deviceId || "");
    db.prepare(
      `
      INSERT INTO algorithm_code_versions
        (workspace_id, id, problem_id, code_blob_id, device_id, language, revision,
         version_kind, label, code_sha256)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
    ).run(
      scope.workspaceId,
      ...
      input.language,
      revision,
      input.versionKind || "autosave",
      (input.label || (revision === 1 ? "初始云端草稿" : "自动保存")).trim().slice(0, 120),
      saved.sha256,
    );
    if (operationId) recordDraftOperation(db, scope, {
      problemId: input.problemId,
    ...
    const head = readDraftHead(db, scope, input.problemId, input.language);
    if (!head) throw new Error("代码草稿保存失败");
    return { codeBlobId: saved.id, sha256: saved.sha256, revision, savedAt: head.updatedAt };
  })();
}
...
export function getAlgorithmDraft(
  ...
  requirePluginEnabled(db, scope, "algorithms");
  requireEditableProblem(db, scope, input.problemId, input.language);
  const row = readDraftHead(db, scope, input.problemId, input.language);
  if (!row) return null;
  return {
    sourceCode: readAlgorithmCodeBlob(db, scope, row.codeBlobId, keys),
...
function readDraftHead(
  ...
  const row = db.prepare(`
    SELECT d.code_blob_id AS codeBlobId, b.sha256, d.revision,
           d.updated_at AS updatedAt, d.last_device_id AS deviceId,
           COALESCE(device.name, '') AS deviceName
    FROM algorithm_code_drafts d
    JOIN algorithm_code_blobs b
      ON b.workspace_id = d.workspace_id AND b.id = d.code_blob_id
    LEFT JOIN algorithm_devices device
      ON device.workspace_id = d.workspace_id AND device.id = d.last_device_id
    WHERE d.workspace_id = ? AND d.problem_id = ? AND d.language = ?
  `).get(scope.workspaceId, problemId, language) as {
    codeBlobId: string;
    sha256: string;
    revision: number;
    updatedAt: string;
...
function normalizeDraftBaseRevision(value: number | undefined, fallback: number): number {
  if (value === undefined) return fallback;
  const revision = Number(value);
  if (!Number.isSafeInteger(revision) || revision < 0) throw new Error("草稿基础修订无效");
  return revision;
}

function normalizeDraftOperationId(value: string | undefined): string {
  if (value === undefined || value === "") return "";
  const operationId = value.trim();
  if (!/^[A-Za-z0-9:_-]{8,160}$/.test(operationId)) throw new Error("草稿幂等键无效");
  return operationId;
}

function recordDraftOperation(
  db: Database.Database,
  scope: WorkspaceScope,
  input: {
    problemId: number;
  ...
  db.prepare(`
    INSERT INTO algorithm_draft_operations
      (workspace_id, problem_id, language, operation_id, base_revision,
       saved_revision, code_blob_id, code_sha256)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    scope.workspaceId,
    input.problemId,
...
export function listAlgorithmCodeVersions(
  db: Database.Database,
  scope: WorkspaceScope,
  input: { problemId: number; language: JudgeLanguage; limit?: number },
  ...
  return db
    .prepare(
      `
    SELECT v.id, v.problem_id AS problemId, v.language, v.revision,
           v.version_kind AS kind, v.label, v.code_sha256 AS sha256,
           COALESCE(d.name, '') AS deviceName, v.created_at AS createdAt
    FROM algorithm_code_versions v
    LEFT JOIN algorithm_devices d
      ON d.workspace_id = v.workspace_id AND d.id = v.device_id
    WHERE v.workspace_id = ? AND v.problem_id = ? AND v.language = ?
    ORDER BY v.revision DESC
    LIMIT ?
  `,
    )
    .all(scope.workspaceId, input.problemId, input.language, limit) as AlgorithmCodeVersion[];
}

export function readAlgorithmCodeVersion(
  db: Database.Database,
  scope: WorkspaceScope,
  versionId: string,
  keys: JudgeCodeKey[],
): { version: AlgorithmCodeVersion; sourceCode: string } {
  requirePluginEnabled(db, scope, "algorithms");
  const row = db
    .prepare(
      `
  ...
  `,
    ...
    .get(scope.workspaceId, versionId) as (AlgorithmCodeVersion & { codeBlobId: string }) | undefined;
  if (!row) throw new Error("代码版本不存在");
  return {
    version: row,
    sourceCode: readAlgorithmCodeBlob(db, scope, row.codeBlobId, keys),
  };
...
export function prepareAlgorithmSubmission(
  ...
  if (existing) {
    ...
    if (
      ...
      (input.sourceTaskId !== undefined && input.sourceTaskId !== attemptContext.source_task_id) ||
  ...
  const maxHintLevel = Math.max(reportedHintLevel, getSessionMaxHintLevel(db, scope, input.sessionId));
...
export function getAlgorithmSubmission(db: Database.Database, scope: WorkspaceScope, id: number): AlgorithmSubmission {
  requirePluginEnabled(db, scope, "algorithms");
  const row = db
    .prepare(
      `
...
function finalizeAttempt(db: Database.Database, scope: WorkspaceScope, attemptId: number, status: JudgeStatus): void {
  const attempt = db
    .prepare(
      `
  ...
  if (!LEARNING_STATUSES.has(status)) {
    db.prepare(
      ...
      WHERE workspace_id = ? AND id = ?
    `,
    ).run(status, Math.ceil(attempt.active_seconds / 60), scope.workspaceId, attempt.id);
    return;
  }
...
function completePendingAlgorithmReview(
  ...
  `,
    ...
    .get(scope.workspaceId, reviewedProblemId, attempt.review_kind, attempt.id) as { id: number } | undefined;
  if (!review) return;
  db.prepare(
    `
  ...
  const row =
    requestedId === null
      ...
      `,
          ...
          .get(scope.workspaceId, requestedId, input.day, String(input.problemId)) as { id: number } | undefined);
...
function requireEditableProblem(
  db: Database.Database,
  scope: WorkspaceScope,
  problemId: number,
  language: string,
  ...
  const row = db
    ...
    SELECT id, title, judge_problem_ref, supported_languages_json
    FROM algorithm_problems
    WHERE workspace_id = ? AND id = ? AND problem_mode IN ('managed', 'imported')
  `,
    )
    .get(scope.workspaceId, normalizedId) as ManagedProblemRow | undefined;
  if (!row) throw new Error("该题尚未提供在线代码工作区");
  const languages = parseLanguages(row.supported_languages_json);
  if (!languages.includes(language as JudgeLanguage)) throw new Error("该题不支持所选语言");
  return row;
}
...
function parseLanguages(value: string): JudgeLanguage[] {
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((language): language is JudgeLanguage => JUDGE_LANGUAGES.includes(language as JudgeLanguage))
      : [];
  } catch {
    return [];
  }
...
function mapSubmission(row: SubmissionRow): AlgorithmSubmission {
  ...
  return {
    ...
    language: JUDGE_LANGUAGES.includes(row.language as JudgeLanguage) ? (row.language as JudgeLanguage) : "cpp17",
