...
describe("reviews repo", () => {
  it("raises mastery and schedules the next review on success", () => {
    ...
    createReviewEvent(db, { day: "2026-07-01", knowledgePointId: "kp1", score: 3 });
  ...
  it("resets the ladder on a failed review", () => {
    ...
    createReviewEvent(db, { day: "2026-07-01", knowledgePointId: "kp1", score: 0 });
  ...
  it("creating a mistake lowers mastery and schedules D+1 review", () => {
    ...
    createMistake(db, { day: "2026-07-01", title: "漏了 bias", knowledgePointId: "kp1", subjectCode: "M1" });
  ...
  it("graduates a mistake when reattempted successfully and logs a review", () => {
    const db = createTestDb();
    seedSubjectWithChapter(db);
    createMistake(db, { day: "2026-07-01", title: "错题", knowledgePointId: "kp1" });

    const result = reattemptMistake(db, { id: 1, day: "2026-07-02", score: 3 });
    ...
    const book = getMistakeBook(db, "2026-07-03");
  ...
  it("keeps failed reattempts in the due queue", () => {
    const db = createTestDb();
    createMistake(db, { day: "2026-07-01", title: "错题" });

    reattemptMistake(db, { id: 1, day: "2026-07-02", score: 1 });
  ...
  it("rejects invalid dates and empty titles", () => {
    const db = createTestDb();
    expect(() => createStudySession(db, { day: "bad", title: "x" })).toThrow();
    expect(() => createStudySession(db, { day: "2026-07-01", title: "  " })).toThrow();
  });
});
