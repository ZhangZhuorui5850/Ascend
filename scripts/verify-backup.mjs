...
import { createReadStream, cpSync, existsSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, rmSync, statSync, unlinkSync, writeFileSync } from "node:fs";
...
function inspectDatabase(filePath, uploadsByPath) {
  ...
  try {
    ...
    if (integrity.length !== 1 || integrity[0] !== "ok") throw new Error(`SQLite integrity_check 失败：${integrity.join(", ")}`);
    const errors = [];
    let assetCount = 0;
    let blobCount = 0;
    if (tableExists(database, "assets") && columns(database, "assets").has("relative_path")) {
      const rows = database.prepare("SELECT id, relative_path, size FROM assets WHERE relative_path IS NOT NULL AND relative_path != ''").all();
